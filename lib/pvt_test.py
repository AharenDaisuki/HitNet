...
class CAB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        super(CAB, self).__init__()
        stackedConv = []
        stackedConv.append(conv(n_feat, n_feat, kernel_size, bias=bias))
        stackedConv.append(act)
        stackedConv.append(conv(n_feat, n_feat, kernel_size, bias=bias))

        self.ca = CALayer(n_feat, reduction, bias=bias) 
        self.c2 = nn.Sequential(*stackedConv)
